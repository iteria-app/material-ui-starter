...
export const EntityDetailContainer: React.FC<EntityDetailContainerProps> = (
  ...
  const handleSubmit = (values: FormikValues) => {
    ...
    const filterOutReadonlyFields = (
      entityFields: Field[],
      values: any
    ) => {
      entityFields.forEach((field, index, arr) => {
        if (field.type === 'object' && field.name.includes('aggregate')) {
          arr.splice(index, 1)
          if(values?.length > 0) {
            values.forEach((value) => delete value[field.name])
          } else delete values[field.name]
        }
        if (field.type === 'array' && field?.entityFields)
          filterOutReadonlyFields(field?.entityFields?.fields, values[field.name])
      })
    }

    filterOutReadonlyFields(eF.entityFields.fields, values)
    const newItem = {
      ...fillFlatEntityFieldValueObject(
