...
import { useEntityByIdQuery, useInsertEntityMutation } from '../../generated/graphql'
import { 
  deepClone,
  Error,
  ErrorBoundary, 
...
const EntityFormContainer: React.FC = () => {
  const { id } = useParams()
  ...
  const handleSubmit = (values: FormikValues) => {
    insertMutation(getUpsertQuery(values, generatedGraphql)).then((response) => {
      if (response.error) {
