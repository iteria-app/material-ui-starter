  ...
  req.on('end', function () {
    ...
    if (filePath) {
      import_fs.default.access(
        filePath,
        import_fs.default.constants.R_OK,
        (err) => {
          if (err) {
          ...
          import_fs.default.unlink(filePath, (err2) => {
            ...
            res.end(
              JSON.stringify({ message: 'File was successfully deleted' })
            )
          })
        }
      )
...
function iteriaLowcode(options) {
  var _a, _b
  const ideDevServerIteriaAppInitConf = __spreadProps(
    __spreadValues(
      __spreadValues(
        __spreadValues(
          {
  ...
  // const ideDevserverPath = `'https://unpkg.com/@iteria-app/ide-devserver@${(_b = options.version) != null ? _b : "1.2.8"}/dist/ide-devserver.es.js'`;
